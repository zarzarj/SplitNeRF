...
from . import diff_env_utils as util
...
class EnvironmentLight(torch.nn.Module):
    ...
    def build_mips(self, cutoff=0.99):
        self.base.data = self.base.clamp(0.0)
    ...
    def get_envmap_img(self):
        return util.cubemap_to_latlong(self.base, [512, 1024])
    
    def get_specular(self, R, roughness):
        # import pdb; pdb.set_trace()
        if R.shape[0] == 0: return torch.zeros_like(R)
        miplevel = self.get_mip(roughness.float()).reshape(1,1,-1,1)
        spec = dr.texture(self.specular[0][None, ...], R.reshape(1,1,-1,3).contiguous(), mip=list(m[None, ...] for m in self.specular[1:]), mip_level_bias=miplevel[..., 0], filter_mode='linear-mipmap-linear', boundary_mode='cube')
        return spec.reshape(*R.shape)
    
    def get_diffuse(self, N):
        # print(N.shape)
        if N.shape[0] == 0: return torch.zeros_like(N)
        # import pdb; pdb.set_trace()
        diffuse = dr.texture(self.diffuse[None, ...], N.reshape(1,1,-1,3).contiguous(), filter_mode='linear', boundary_mode='cube')
        return diffuse.reshape(*N.shape)
