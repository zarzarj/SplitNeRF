...
from models.utils_illum import get_sph_sample_coords
...
@models.register('neus-split')
class NeuSSplitModel(BaseModel):
    def setup(self):
    ...
    def forward_(self, rays):
        ...
        # texture_out = self.texture(feature, t_dirs, normal)
        texture_out = self.texture(feature, t_dirs, normal, positions)
        rgb = texture_out["color"]
        ...
        if self.texture.use_transparency:
            Kt = texture_out["Kt"]
            alpha_trans = Kt * alpha
            weights_trans = render_weight_from_alpha(alpha_trans, ray_indices=ray_indices, n_rays=n_rays)
            comp_rgb = accumulate_along_rays(weights_trans, ray_indices, values=rgb, n_rays=n_rays)
            # alpha = alpha * Kt
        ...
        if self.training:
            out.update({
                ...
                'ray_indices': ray_indices.view(-1),
                # "local": texture_out["local"],
                "pos": positions,
                # "ao": texture_out["ao"],
                # "roughness": texture_out["roughness"],
                "normals": normal,
            })
            out.update(texture_out)
            if self.config.geometry.grad_type == 'finite_difference':
                out.update({
        ...
        if not self.training:
            extras = ["local", "ao", "roughness", "metallic", "albedo"]
            if self.texture.use_transparency: extras.append("transparency")
            extras_comp = {}
            for name, v in texture_out.items():
                if name in extras:
                    extras_comp[f"extras_comp_{name}"] = accumulate_along_rays(weights, ray_indices, values=v, n_rays=n_rays)
            out.update(extras_comp)
            out_full.update({
                'extras_comp_albedo': extras_comp['extras_comp_albedo'] + out_bg['comp_rgb'] * (1.0 - out['opacity'])
            })
    ...
    def export(self, export_config):
        ...
        meshes = {}
        to_save = ["albedo", "ao", "roughness", "metallic"]
        if export_config.export_vertex_color:
            ...
            normal = F.normalize(sdf_grad, p=2, dim=-1)
            out = self.texture(feature, -normal, normal)
            for name, value in out.items():
                if name not in to_save: continue
                if name == "ao": value = value.mean(dim=-1, keepdim=True)
                if value.shape[-1] == 1: value = value.repeat(1,3)
                meshes[name] = copy.deepcopy(mesh)
                meshes[name]['v_rgb'] = value.cpu()
        return meshes
    
    @torch.no_grad()
    def export_envmap(self, export_config):
        elev_res, az_res = export_config.res_h, export_config.res_w
        sample_coords = get_sph_sample_coords(elev_res, az_res)

        out = {}
        diff_global = self.texture.get_diffuse_global(sample_coords)
        diff_im = diff_global.reshape(elev_res, az_res, 3)
        out['diffuse'] = diff_im

        for roughness in export_config.specular_samples:
            roughness_vec = torch.ones(sample_coords.shape[0], 1).to(self.rank) * roughness
            spec_global = self.texture.get_specular_global(sample_coords, roughness_vec)
            spec_im = spec_global.reshape(elev_res, az_res, 3)
            out[f'specular_{roughness}'] = spec_im
            
        return out
    
    def update_ao_mesh(self, optimizer):
        mesh = self.isosurface()
        if self.config.indirect_light:
            self.texture.update_ao_mesh(mesh, self.geometry, optimizer)
        else:
            self.texture.update_ao_mesh(mesh, None, optimizer)
